...
describe('TableComponent', () => {
  ...
  it('should have a header displayed with the correct content', () => {
    const matRows = component.nativeElement.querySelectorAll('tbody tr');
    expect(matRows.length).toBe(2);
    expect(matRows[0].innerText.trim()).toContain('Mike');
    expect(matRows[0].innerText.trim()).toContain('CLICKME');
    expect(matRows[1].innerText.trim()).toContain('Tilly');
    expect(matRows[1].innerText.trim()).toContain('CLICKME');
  });

  it('should have a row with correct header content', () => {
    const matHeaderCell = component.nativeElement.querySelectorAll('th');
  ...
  it('should have a button in a tablecell', () => {
    const matButtons = component.nativeElement.querySelectorAll('tbody .nes-btn');
  ...
  it('should use the bq-heading to display header', () => {
    // Verify
    expect(component.nativeElement.querySelector('.title')).toBeTruthy();
  });
});
