...
describe('TimelineComponent', () => {
  ...
  it('should render the correct entry date', () => {
    const date = component.nativeElement.querySelectorAll('.datetime');
    expect(date.length).toBe(2);
    expect(date[0].innerHTML).toContain(fiveDecember2017.format(DEFAULT_DATE_FROM_NOW_FORMAT));
    expect(date[1].innerHTML).toContain(now.fromNow(false));
  });

  it('should render the correct timestamp', () => {
    const time = component.nativeElement.querySelectorAll('.datetime');
    expect(time.length).toBe(2);
    expect(time[0].innerHTML).toContain('17:00');
    expect(time[1].innerHTML).toContain(now.format('HH:mm'));
  });
  ...
  it('should display header', () => {
    // Verify
    expect(component.nativeElement.querySelector('.title').innerText).toBe('statisticContainer');
  });
