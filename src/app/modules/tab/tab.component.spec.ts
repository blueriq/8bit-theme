...
describe('TabComponent', () => {
  ...
  it('should display the correct tab headers', () => {
    const tabButtons = tabFixture.nativeElement.querySelectorAll('.nes-btn');

    expect(tabButtons.length).toBe(4);
    expect(tabButtons[0].innerText).toEqual('News');
    expect(tabButtons[1].innerText).toEqual('Reviews');
    expect(tabButtons[2].innerText).toEqual('Pricewatch');
    expect(tabButtons[3].innerText).toEqual('[tab4]');
  });

  it('should have 4 containers rendered', () => {
    const tabBodies = tabFixture.nativeElement.querySelectorAll(':scope > div');

    expect(tabBodies.length).toBe(4);
    expect(tabBodies[0].querySelector('p')).toBeTruthy('The first container should be displayed');
  });
  ...
  it('should display header', () => {
    // Verify
    expect(tabFixture.nativeElement.querySelector('h2')).toBeTruthy();
  });
